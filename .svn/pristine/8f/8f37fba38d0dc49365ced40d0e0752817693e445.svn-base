...
Ext.define('Rich.view.flight.AirMain', {
	...
	initComponent:function(){
		...
		var action = [{
            ...
            icon: Rich.Url.icons_48,
            iconCls:'actioncolumn-margin',
            altText:'预览文档',
            tooltip: '预览文档',
            handler: function(gridview, rowIndex, colIndex) {
                var rec = gridview.getStore().getAt(rowIndex);
                this.up('gridpanel').toSee(rec);
            }
       	}]
			  	...
			  	columns: {
				   	...
				   	items:[
				        ...
				        {text: '任务时间', dataIndex: 'beginTime',flex: 1,renderer:function(v,record){
				        	var begin = v.substring(0,8);
				        	var endtime = record.record.data.endTime.substring(0,8);
