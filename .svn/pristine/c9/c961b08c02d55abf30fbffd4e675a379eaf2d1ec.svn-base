...
public class UserController{
	...
	public WebResponse userPerUpdate(UpdateUserPerParam param)throws Exception {
		ValidateUtil.validateGroup(param, First.class);
		String userId = WechatAppContextHelper.getCurrentUser().getUserId();
	...
	@ResponseBody
	@Login
	public WebResponse updateUserPhone(@RequestBody UpdateUserPhoneParam param)throws Exception {
		ValidateUtil.validate(param);
		String phone = iSmsService.verifyCode(param.getCode());
		WxSessionUser currentUser = WechatAppContextHelper.getCurrentUser();
		int result = iUserServiceCore.updateUserPhone(currentUser.getUserId(), currentUser.getType(), phone);
		if(result==1)iSmsService.removeCode();
		return new WebResponse();
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/user/per/auth", method = RequestMethod.PUT)
	@ResponseBody
	@Login
	public WebResponse updateUserAuth(@RequestBody UserPerUpdateAuthParam param)throws Exception {
		ValidateUtil.validate(param);
		WxSessionUser currentUser = WechatAppContextHelper.getCurrentUser();
		return new WebResponse(iUserServiceCore.updateUserPerAuth(currentUser.getUserId(), param));
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/user/com/auth", method = RequestMethod.PUT)
	@ResponseBody
	@Login
	public WebResponse updateComUserAuth(@RequestBody UserComUpdateAuthParam param)throws Exception {
		ValidateUtil.validate(param);
		WxSessionUser currentUser = WechatAppContextHelper.getCurrentUser();
		return new WebResponse(iUserServiceCore.updateUserComAuth(currentUser.getUserId(), param));
	}
