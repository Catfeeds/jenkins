...
import com.hfocean.common.validation.custom.IdCardValidator;
import com.hfocean.common.validation.custom.IdcardInfoExtractor;
...
public class UserServiceCoreImpl extends BaseServiceImpl<UserPersonalPO,String> implements IUserServiceCore {
	...
	public UserPersonalPO saveUserPerRgister(UserPerRegisterParam param)throws Exception {
		...
		//转化出生日期
		if(idCard!=null){
			IdcardInfoExtractor idCardInfo = IdCardValidator.getIdCardInfo(idCard);
//		Date birthDay = getBirthDay(param.getBirthday());
			user.setBirthday(idCardInfo.getBirthday());
			//判断性别是否传入M,F
//			String sex = getSex(param.getSex());
			user.setSex(idCardInfo.getGender());
		}
		//设置密码
	...
	public UserPersonalPO updateUserPer(String userId, UpdateUserPerParam param) throws Exception {
		...
		if(user!=null){
			...
			if(authStatus==null||!authStatus.equals(UserEnum.已认证.getCode())){
				...
				IdcardInfoExtractor idCardInfo = IdCardValidator.getIdCardInfo(param.getIdCard());
				user.setName(param.getName());
				//判断性别是否传入M,F
//				String sex = getSex(param.getSex());
				user.setSex(idCardInfo.getGender());
...
//				Date birthDay = getBirthDay(param.getBirthday());
				...
				if(idCard!=null&&!idCard.trim().equals("")){
					...
					if(userIdCard==null||!userIdCard.equals(idCard)){
						verifyPhone(idCard);
						user.setIdCard(idCard);
					}
				}
		...
		if(user!=null){
			...
			if(authStatus==null||!authStatus.equals(UserEnum.已认证.getCode())){
				...
				if(companyName!=null&&!companyName.trim().equals("")){
					...
					String userCompanyName = user.getCompanyName();
					if(userCompanyName==null||!userCompanyName.equals(companyName)){
						//验证公司名称唯一
						verifyCompanyName(companyName);
						user.setCompanyName(companyName.trim());;
					}
				}
				String licenseNumber = param.getLicenseNumber();
				if(licenseNumber!=null&&!licenseNumber.trim().equals("")){
					licenseNumber = licenseNumber.trim();
					String userLicenseNumber = user.getLicenseNumber();
					if(userLicenseNumber==null||!userLicenseNumber.equals(licenseNumber)){
						//社会信用代码唯一
	...
	public int updateUserPerAuth(String userId, UserPerUpdateAuthParam param) throws Exception {
		...
		String idCard = param.getIdCard();
		String userICard = perUser.getIdCard();
		if(userICard==null||!idCard.trim().equals(userICard)){
			//判断身份证是否唯一
			verifyPhone(idCard);
			perUser.setIdCard(idCard);
		}
		//修改
		...
		IdcardInfoExtractor idCardInfo = IdCardValidator.getIdCardInfo(idCard);
		perUser.setBirthday(idCardInfo.getBirthday());
		perUser.setSex(idCardInfo.getGender());
	...
	public int updateUserComAuth(String userId, UserComUpdateAuthParam param) throws Exception {
		...
		if(companyName!=null&&!companyName.trim().equals("")){
			companyName = companyName.trim();
			String userCompanyName = comUser.getCompanyName();
			if(userCompanyName==null||!userCompanyName.equals(companyName)){
				//验证公司名称唯一
				verifyCompanyName(companyName);
				comUser.setCompanyName(companyName.trim());;
			}
		}
		String licenseNumber = param.getLicenseNumber();
		if(licenseNumber!=null&&!licenseNumber.trim().equals("")){
			licenseNumber = licenseNumber.trim();
			String userLicenseNumber = comUser.getLicenseNumber();
			if(userLicenseNumber==null||!userLicenseNumber.equals(licenseNumber)){
				//社会信用代码唯一
				verifyLicenseNumber(licenseNumber);
				comUser.setLicenseNumber(licenseNumber);
			}
		}
		//修改
