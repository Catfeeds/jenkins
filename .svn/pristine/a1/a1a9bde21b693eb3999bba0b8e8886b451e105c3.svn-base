<template>
	<div class="plan" v-loading="isLoading" element-loading-text="数据提交中，请稍候...">
		...
		<div class="plan-form">
			<el-form ref="flyplan" label-position="left" :model="flyplan" :rules="rules" label-width="130px">
				...
				<div class="plan-form-content">
					...
					<el-form-item label="计划类型" prop="planType" auto-complete="off">
						<el-radio-group v-model="flyplan.planType" @change="flyTypeChange">
							...
							<el-input style="margin-left:10px;width:150px;" :disabled="planTypeInputDisable" v-model="flyplan.planTypeName"></el-input>
						</el-radio-group>

					</el-form-item>
					<!--使用机型-->
					<el-form-item label="使用机型" class="clear" prop="planes" auto-complete="off">
						<el-checkbox-group v-for="plane in flyplan.planes" class="left" style="margin-bottom:5px;" v-model="plane.checked">
							<el-checkbox style="margin-right:11px;" :label="plane.name"></el-checkbox>
							<el-input v-if="plane.name=='其他'" :disabled="!plane.checked" style="width:150px;" v-model="plane.desc" placeholder="请输入内容"></el-input>
							<el-input-number :disabled="!plane.checked" style="width:120px;margin-right:10px;" v-model="plane.number" :min="0" :max="99"></el-input-number>
						</el-checkbox-group>
					</el-form-item>
					<el-form-item label="使用时间" prop="planTime">
						<el-date-picker v-model="flyplan.planTime.date" type="date" placeholder="选择日期" readonly :picker-options="pickerOptions" style="z-index: -1;">
						</el-date-picker>
						<el-select style="width:90px;" v-model="flyplan.planTime.beginHour" placeholder="请选择">
							<el-option v-for="hour in selectableFlyBeginHours" :label="hour.label" :value="hour.label"></el-option>
						</el-select>
						<el-select style="width:90px;" v-model="flyplan.planTime.beginMinute" placeholder="请选择">
							<el-option v-for="minute in selectableFlyBeginMinutes" :label="minute.label" :value="minute.label"></el-option>
						</el-select>
						<strong>至</strong>
						<el-select style="width:90px;" v-model="flyplan.planTime.endHour" placeholder="请选择">
							<el-option v-for="hour in selectableFlyEndHours" :label="hour.label" :value="hour.label"></el-option>
						</el-select>
						<el-select style="width:90;" v-model="flyplan.planTime.endMinute" placeholder="请选择">
							<el-option v-for="minute in selectableFlyEndMinutes" :label="minute.label" :value="minute.label"></el-option>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="计划空域及高度" prop="airInfo">
						<div v-for="(airInfo,index) in flyplan.airInfo" style="border-bottom:dashed 1px #f0f0f0;margin-bottom:10px;">
							<el-form-item label="位置描述" class="airinfo_item">
								<el-input style="width:300px;" v-model="airInfo.location" placeholder="请输入内容"></el-input>
								<el-button type="text" @click="removeAirInfo(index)" style="color:#ff0000;float:right;margin-right:100px;">删除
								</el-button>
							</el-form-item>
							...
							<el-form-item label="飞行范围" class="airinfo_item">
								<el-select style="width:120px;" v-model="airInfo.scopeType" placeholder="请选择" class="airinfo_item">
									<el-option :value=1 label="圆形空域"></el-option>
									<el-option label="多边形空域" :value=2></el-option>
								</el-select>
								...
								<div v-if="airInfo.scopeType=='1'">
									<div class="airinfo_item clear">
										...
										<div class="left" style="margin-right:10px;margin-bottom: 10px;">
											...
											<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="airInfo.lng0" :min="-180" :max="180"></el-input-number>
											<span>度</span>
											<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="airInfo.lng1" :min="0" :max="60"></el-input-number>
											<span>分</span>
											<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="airInfo.lng2" :min="0" :max="60"></el-input-number>
											<span>秒</span>
										</div>
										<div class="left" style="margin-left: 130px; ">
											<span style="background:#169bd5;padding:5px;">纬</span>
											<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="airInfo.lat0" :min="-90" :max="90"></el-input-number>
											<span>度</span>
											<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="airInfo.lat1" :min="0" :max="60"></el-input-number>
											<span>分</span>
											<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="airInfo.lat2" :min="0" :max="60"></el-input-number>
											<span>秒</span>
										</div>
									...
									<div>
										...
										<el-input-number :controls="false" :debounce="0" style="width:80px;margin-left: 80px;" v-model="airInfo.radius" :min="0"></el-input-number>
										米。
									</div>
								</div>
								<div v-if="airInfo.scopeType=='2'">
									...
									<div v-for="(center,index) in airInfo.centers" class="airinfo_item">
										<div class="left" style="margin-right:10px;">
											<span style="background:#169bd5;padding:5px;">经</span>
											<el-input-number :controls="false" :debounce="0" style="width:60px;" v-model="center.lng0" :min="-180" :max="180"></el-input-number>
											<span>度</span>
											<el-input-number :controls="false" :debounce="0" style="width:60px;" v-model="center.lng1" :min="0" :max="60"></el-input-number>
											<span>分</span>
											<el-input-number :controls="false" :debounce="0" style="width:60px;" v-model="center.lng2" :min="0" :max="60"></el-input-number>
											<span>秒</span>
										</div>
										<div class="left" style="margin-right:10px;">
											<span style="background:#169bd5;padding:5px;">纬</span>
											<el-input-number :controls="false" :debounce="0" style="width:60px;" v-model="center.lat0" :min="-90" :max="90"></el-input-number>
											<span>度</span>
											<el-input-number :controls="false" :debounce="0" style="width:60px;" v-model="center.lat1" :min="0" :max="60"></el-input-number>
											<span>分</span>
											<el-input-number :controls="false" :debounce="0" style="width:60px;" v-model="center.lat2" :min="0" :max="60"></el-input-number>
											<span>秒</span>
										</div>
					...
					<el-form-item label="主起降场" prop="landingInfo">
						...
						<div v-for="(landingInfo,index) in flyplan.landingInfo" style="border-bottom:dashed 1px #f0f0f0;margin-bottom:50px;">
							<el-form-item label="名称" class="landing_item">
								<el-select style="width:120px;" v-model="landingInfo.id" placeholder="请选择" @change="handleChangeLanding(landingInfo)">
									<el-option v-for="landing in landings" :label="landing.name" :value="landing.id"></el-option>
								</el-select>
								<el-button type="text" @click="removeLanding(index)" style="color:#ff0000;float:right;margin-right:100px;">删除
								</el-button>
							</el-form-item>
							<el-form-item label="起降场位置描述" v-if="landingInfo.id==-1" class="landing_item">
								<div style="margin-left:0px;">
									<el-input style="width:400px;" v-model="landingInfo.location" placeholder="请输入内容"></el-input>
							...
							<el-form-item label="起降场坐标" v-if="landingInfo.id==-1" class="landing_item">
								<div style="margin-left:0px;">
									<div class="left">
										<span style="background:#169bd5;padding:5px;">经</span>
										<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="landingInfo.lng0" :min="-180" :max="180"></el-input-number>
										<span>度</span>
										<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="landingInfo.lng1" :min="0" :max="60"></el-input-number>
										<span>分</span>
										<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="landingInfo.lng2" :min="0" :max="60"></el-input-number>
										<span>秒</span>
									</div>
									<div class="left" style="margin-top:10px;margin-left:129px ;">
										<span style="background:#169bd5;padding:5px;">纬</span>
										<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="landingInfo.lat0" :min="-90" :max="90"></el-input-number>
										<span>度</span>
										<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="landingInfo.lat1" :min="0" :max="60"></el-input-number>
										<span>分</span>
										<el-input-number :controls="false" :debounce="0" style="margin-right:10px;width:50px;" v-model="landingInfo.lat2" :min="0" :max="60"></el-input-number>
										<span>秒</span>
									</div>
								</div>
								<el-form-item label="备用起降场" prop="" style="width: 300px;position: absolute; left: -132px; top:100px" >
					...
					<el-row>
						<el-col :span="12">
							<el-form-item label="单位联系人" prop="contactInfo.unitContactName">
								<el-input style="width:150px;" v-model="flyplan.contactInfo.unitContactName" placeholder="请输入内容"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="联系电话" prop="contactInfo.unitContact">
								<el-input style="width:150px;" v-model="flyplan.contactInfo.unitContact" placeholder="请输入内容"></el-input>
							</el-form-item>
						</el-col>
					</el-row>
					<el-row>
						<el-col :span="12">
							<el-form-item label="现场联系人" prop="contactInfo.fieldContactName">
								<el-input style="width:150px;" v-model="flyplan.contactInfo.fieldContactName" placeholder="请输入内容"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="联系号码" prop="contactInfo.fieldContactPhone">
								<el-input style="width:150px" v-model="flyplan.contactInfo.fieldContactPhone" placeholder="请输入内容"></el-input>
							</el-form-item>
						</el-col>
					</el-row>
					<!--
				...
				<el-form-item>
					<el-button type="primary" @click="submit" style="padding:10px 50px;">提交
					</el-button>
				</el-form-item>
			</el-form>
		</div>
	...
	export default {
		...
		data() {
			...
			var planPhoneValidator = (rule, value, callback) => {
				...
				if(!myreg.test(this.flyplan.contactInfo.unitContact)) {
			...
			var emergencyContactPhoneValidator = (rule, value, callback) => {
				if(this.flyplan.contactInfo.fieldContactPhone == this.flyplan.contactInfo.unitContact) {
					return callback(new Error(rule.message));
				}
				callback();
			};
			...
			return {
				...
				rules: {
					...
					'airInfo': [{
						...
						message: '空域位置不能为空',
						trigger: 'blur',
						validator: airInfoValidator
					}],
					'landingInfo': [{
						required: true,
						message: '起降场位置不能为空',
						trigger: 'blur',
						validator: landingInfoValidator
					}],
					...
					'contactInfo.fieldContactPhone': [{
							required: true,
							message: '请输入现场联系人手机号码',
							trigger: 'blur'
						},
						{
							validator: planContactPhone,
							min: 11,
							max: 11,
							trigger: 'blur'
						},
						{
							...
							message: '单位联系人的电话号码不能与现场联系人的电话号码一样',
							trigger: 'blur'
						}
					],
					...
					'contactInfo.unitContact': [{
							required: true,
							message: '请输入单位联系人手机号码',
							trigger: 'blur'
						},
						{
							min: 11,
							max: 11,
							validator: planPhoneValidator,
							trigger: 'blur'
						}
