...
Ext.define('Rich.view.flight.ApplyDetail',{
    ...
    setV:function(rec){
    	...
    	/*if(landingInfo.slave && landingInfo.slave.id == -1){
    		var lng = this.lookupI('lng');
    		lng.setValue('经度:'+landingInfo.slave.lng+' 纬度:'+landingInfo.slave.lat)
    		lng.show();
    	};*/
						 	...
						 	items:[{    
								...
								createItemByValue:function(v){
									...
									if(ce){
										...
										var re = result.substring(0,result.length-2);
										var da =  dat.substring(0,dat.length-2);
										
									}
									var lnglat = a != null?('经度:'+a.lng+',纬度:'+a.lat):(re+da);
									return {
