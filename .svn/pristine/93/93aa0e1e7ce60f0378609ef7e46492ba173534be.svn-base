		        ...
		        viewConfig:{
			  	...
			  	columns: {
				   	...
				   	items:[
				        ...
				        { text: '申请人/单位', dataIndex: 'userName',flex: 1 },
				        ...
				        { text: '联系人及联系方式', dataIndex: 'contactInfo',hidden:true,width: 120 ,renderer:function(v,record){
					        	...
					        	if(record.record.data.type == 0){
					        		return contact.fieldContactName+' ('+contact.fieldContactPhone+')';
					        	}else if(record.record.data.type == 1){
					        		return contact.fieldContactName+' ('+contact.fieldContactPhone+')';
					        	}else if(record.record.data.type == 2){
					        		return contact.controller+' ('+contact.controllerContact+')';
					        	}
				        ...
				        { text: '计划执行动态', dataIndex: 'startTime',flex: 1,hidden:true,renderer:function(v,record){
				        	var begin = v?v.substring(9,14):null;
				        	var end = record.record.raw.finishTime?record.record.raw.finishTime.substring(9,14):null;
				        	//;
				        	if(begin != '' && begin != null && end != '' && end != null){
				        		return '起飞:'+begin+', 降落:'+end;
				        	}else if((begin != '' && begin != null) && (end == '' || end == null)){
				        		return '起飞:'+begin+', -';
				        	}else{
				        		return '-';
				        	}
