		...
		itms.unshift({
			...
			items:[{
				...
				handler:function(btn){
					Ext.create('Rich.view.data.DataAdd').showFor(this.returnFn,this);
				},
				returnFn:function(){
					Rich.Msg.alert('消息','新增成功!')
					this.up('datatmain').lookupI('list').getStore().reload();
				}
			}]
		});
		...
		Ext.apply(me,{
			...
			items:[{
				...
				lookDetail:function(rec){
					Ext.create('Rich.view.data.DataDetail',{rec:rec.data}).showFor(this.authBack,this);
				},
				authBack:function(o,f,r){
					if(f){
