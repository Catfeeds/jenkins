...
//			iSmsService.sendSMS(findOne.getPhone(), SmsTemplate, params)
	...
	@Override
	public int deleteUserPer(String userId) throws Exception {
		UserPersonalPO per = userPersonalRepository.findOne(userId);
		if(per!=null){
			//查询微信绑定
			List<UserWxPO> wxPerList = userWxRepository.findByUserIdAndType(userId, UserEnum.个人用户.getCode());
			if(wxPerList!=null&&wxPerList.size()>0){
				for(UserWxPO wxPer : wxPerList){
					userWxRepository.delete(wxPer);
				}
			}
			userPersonalRepository.delete(per);
		}
		return 1;
	}

	@Override
	public int deleteUserCom(String companyId) throws Exception {
		UserCompanyPO com = userCompanyRepository.findOne(companyId);
		if(com!=null){
			//查询微信绑定
			List<UserWxPO> wxPerList = userWxRepository.findByUserIdAndType(companyId, UserEnum.企业用户.getCode());
			if(wxPerList!=null&&wxPerList.size()>0){
				for(UserWxPO wxPer : wxPerList){
					userWxRepository.delete(wxPer);
				}
			}
			userCompanyRepository.delete(com);
		}
		return 1;
	}
