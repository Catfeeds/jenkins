...
public class OssController {
    ...
    @ResponseBody
    @RequestMapping("/register/attachment")
    public WebResponse uploadFileByBase64(String file,String fileName) throws Exception{
        VerdictUtil.assertNotNull(file);
        VerdictUtil.assertNotNull(fileName);
        ossService.allowIf(fileName);
        return new WebResponse(ossService.simpleUpload(new ByteArrayInputStream(new BASE64Decoder().decodeBuffer(file)), fileName));
    }
}
