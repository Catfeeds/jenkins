					...
					<el-tab-pane label="修改注册手机" name="third">
						<div class="plan-form">
							<el-form label-position="left" label-width="120px" :model="company" ref="phoneComForm" :rules="rules3">
								...
								<div class="plan-form-content">
									<el-form-item label="新注册手机号" prop="newPhone" style="width:40%;" placeholder="验证短信将会发送到此手机上">
										<el-input v-model="company.newPhone"></el-input>
									</el-form-item>
								</div>
								<div class="plan-form-content move_box">
									<el-form-item label="验证码" style="width:40%;" prop="PhoneCheck">
										<el-input v-model="company.PhoneCheck"></el-input>
									</el-form-item>
								</div>
								<el-button type="primary" :disabled="true"  :class="tongyi?'None':'Block'" style="top: 58px;left:300px;position: absolute;">{{timeback}}</el-button>
								<el-button type="primary" :class="tongyi?'Block':'None'" style="top: 58px;left:290px;position: absolute;" @click=checkTel>发送验证码</el-button>
								<el-form-item>
	...
	export default {
		methods: {
			...
			checkTel() {			
				...
				if(this.company.newPhone != "" && this.company.newPhone.length == 11) {
					if(this.company.emergencyContactPhone== this.company.newPhone){
						this.company.newPhone='';
						return this.$message.error('新注册号码不能与紧急联系人号码一致');			
					}
					var _this = this;
					$.ajax({
						type: 'POST',
			...
			updateComPage3() {
				...
				this.isLoading = true;
				var _this = this;		
				if(_this.company.newPhone != "") {
					$.ajax({
						type: 'PUT',
						url: urls.UPDATE_USERPHONE,
						data: { code: _this.company.PhoneCheck /*验证码*/ },
						success: function() {
							_this.$message({
								message: '修改成功',
								type: 'success'
...
//				this.company.licensePic = URL.createObjectURL(file.raw);
				this.company.licensePic = res.data[0].fullUrl;
			},
			/**
			 * 处理文档上传失败
			 */
...
//			var companyNameValidator = (rule, value, callback) => {
//				if(this.company.licensePic == '') {
//					return callback(new Error(rule.message));
//				}
//				callback();
...
//						{ required: true, validator: companyNameValidator, message: '单位名称一致，不能重复提交', trigger: 'blur' }
					],
					companyLegal: [
