			       	...
			       	doRequest:function(params){
					    this.loadPage(1,params,{
						    ...
						    callback: function(reds, o, f){
						    	if(f){
							        this.orgId = o.params.orgId;
									this.orgName = o.params.orgName;
									this.setTitle(o.params.orgName+' > 用户');
						    	}
						    }
						});
			       		return false;
				...
				items:[{
			       	...
			       	doRequest:function(params){
					    this.loadPage(1,params,{
						    ...
						    callback: function(records, o, f) {
						    	if(f){
						    		this.params = params;
									this.setTitle(o.params.nickName+' > 角色');
						    	}
						    }
						});
			       		return false;
					...
					columns: {
					   	...
					   	items:[
					        ...
					        { text: '创建时间', dataIndex: 'createTime',rightId:Rich.V.display_create_time,rightHidden:true,width:140 ,renderer:function(v){					   
					        	if(v != null && v != ''){
					       			var date = new Date(v);
									Y = date.getFullYear() + '-';
