...
import { Cells, CellsTitle, FormCell, CellHeader, Cell, CellBody, CellFooter, Label, ButtonArea, Button, Select, Input } from 'react-weui'
...
export default class UserInfoApp extends React.Component{
	...
	componentDidMount(){
		...
		.then( json => {
			...
			this.setState( Object.assign( {}, this.state, { [stateName] : json.data, type : json.data.type } ) )
		})
	}
	...
	redirectUpdatePhone(){
		Tool.to(`/user/modifyphone`)
	}
	redirectAuth(){
		Tool.to(`/user/auth/${ this.state.type }`)	
	}
	render(){
		let {
				perInfo,
			...
			authStatus;
		if( perInfo ){
			authStatus = perInfo.authStatus
		}else if( comInfo ){
			authStatus = comInfo.authStatus
		}
			...
			contentInfo = (
				<div className = "mutat_edit">
					<Cells>
						...
						{
							<Cell onClick = { () => this.redirectAuth() } access className = "mutat">
								<CellHeader><Label>认证状态</Label></CellHeader>
								<CellBody></CellBody>
								<CellFooter>{ _authStatus }</CellFooter>
							</Cell>
						}
					</Cells>
					...
					<Cells>
						<FormCell className = "mutat">
							...
							<CellFooter onClick = { this.redirectUpdatePhone } style = {{ color : "#31b0d5", fontSize : 12, lineHeight : "14px" }}>更换手机</CellFooter>
						</FormCell>
						<FormCell className = "mutat">
