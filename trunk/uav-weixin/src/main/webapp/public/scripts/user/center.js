				...
				<Panel>
					<PanelBody>
						<MediaBox type = "appmsg">
							...
							<MediaBoxBody>
								<CellBody className = "username" onClick = { this.redirectEdit }>{ name } <img src = { editBS64 }/></CellBody>
								<CellBody className = "mobile">{ phone }</CellBody>
								<CellBody className = "cer" onClick = { () => this.redirectAuth() }><span>{ auth_status[ authStatus ] }</span></CellBody>
							</MediaBoxBody>
						</MediaBox>
					</PanelBody>
				</Panel>
