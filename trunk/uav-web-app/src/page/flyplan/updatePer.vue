							...
							<el-form label-position="left" label-width="120px" :model="person" :rules="rules3" ref="phoneForm">
								...
								<div class="plan-form-content move_box" >
									<el-form-item label="验证码" style="width:40%;" prop="PhoneCheck">
										<el-input v-model="person.PhoneCheck"></el-input>
									</el-form-item>
								</div>
								<el-button type="primary"  :disabled="true"  :class="tongyi?'None':'Block'" style="top: 78px;left:314px;position: absolute;">{{timeback}}</el-button>
								<el-button type="primary" class="right" style="top: 78px;left:306px;position: absolute;" @click=checkTel :class="tongyi?'Block':'None'">发送验证码</el-button>
								<el-form-item>
			...
			updatePerson3() {
				...
				if(this.person.newPhone != "") {
						$.ajax({
							...
							data: { code: _this.person.PhoneCheck /*验证码*/ },
							success: function() {
								_this.$message({
									message: '修改成功',
									type: 'success'
...
//				this.person.idcardPic1 = URL.createObjectURL(file.raw);
				this.person.idcardPic1 = res.data[0].fullUrl;

			},
			handleAvatarSuccess2(res, file) {
//				this.person.idcardPic2 = URL.createObjectURL(file.raw);
				this.person.idcardPic2 = res.data[0].fullUrl;
			},
			handleAvatarSuccess3(res, file) {
//				this.person.idcardPic3 = URL.createObjectURL(file.raw);
				this.person.idcardPic3 = res.data[0].fullUrl;
			},
